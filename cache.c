...
char *make_block(int block_size) {
  ...
  char* blockp = (char*)malloc(block_size*sizeof(char));
...
Line *make_lines(int line_count, int block_size) {
  ...
  Line* linep = (Line*)malloc(line_count*sizeof(Line));
  for(int i = 0; i < line_count; ++i){
    (linep + i)->accessed = make_block(block_size);
    (linep + i)->tag = 0;
    (linep + i)->block_size = block_size;
    (linep + i)->valid = 0;
  }
  return linep;
}
...
Set *make_sets(int set_count, int line_count, int block_size) {
  ...
  //
  Set* setp = (Set*)malloc(set_count*sizeof(Set));
  for(int i = 0; i < set_count; ++i){
     (setp+i)->lines = make_lines(line_count, block_size);
     (setp+i)->line_count = line_count;
     (setp+i)->lru_queue = NULL;
  }
  return setp;
}
